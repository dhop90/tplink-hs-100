...
metadata {
	definition (name: "tplink-hs-100", namespace: "dhop90", author: "David Hopson") {
		capability "Switch"
        capability "Refresh"
        capability "Polling"
	}
...
def hubActionResponse(response){
  	...
  	message("hubActionResponse: ${response}")
    ...
    log.debug "hubActionResponse - parsing"
    //def msg = parseLanMessage(response)
    //message("Parse msg = ${msg}")
